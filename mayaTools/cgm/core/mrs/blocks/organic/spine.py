...
def prerig(self):
    ...
    ml_jointHandles = []
    l_scales = []
    for mHandle in self, mEndHandle:
    ...
    _offsetDist = DIST.get_distance_between_points(_pos_me,_pos_end) / (self.numberControls - 1)
    _l_pos = [ DIST.get_pos_by_vec_dist(_pos_me, _vec, (_offsetDist * i)) for i in range(self.numberControls - 1)] + [_pos_end]
    ...
    #Push scale back...
    for i,mHandle in enumerate([self, mEndHandle]):
    ...
    #...wire some controls
    _arg = "{0}.out_vSplit = {1} + {2} + 1".format(targets[0],
                                                  self.getMayaAttrString('numberJoints','short'),
                                                  self.getMayaAttrString('loftSplit'))
