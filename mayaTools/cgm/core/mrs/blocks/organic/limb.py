...
def define(self):
    ...
    _size = MATH.average(self.baseSize)
    _crv = CURVES.create_controlCurve(self.mNode, shape='jack', 
                                      direction = 'z+', sizeMode = 'fixed', size = _size)
    ...
    return
    log.debug("|{0}| >>  globalAxis helpers...".format(_str_func))
    
    _arrowUp = CURVES.create_fromName('arrowForm', _size/2 )
    CORERIG.override_color(_arrowUp,'green')
    _arrowAim = CURVES.create_fromName('arrowForm', _size/2 )
    CORERIG.override_color(_arrowAim,'blue')
    p_crv = DIST.get_pos_by_axis_dist(self.mNode,'z+', _size * 2)
    
    for i,crv in enumerate([_arrowAim,_arrowUp]):
        mCrv = cgmMeta.validateObjArg(crv)
        mCrv.doStore('cgmName',self.mNode)        
        if i == 0:
            mCrv.doStore('cgmType','blockAimAxis')
        else:
            mCrv.doStore('cgmType','blockUpAxis')
        mCrv.doName()        
        
        mCrv.p_position = p_crv
        CORERIG.copy_pivot(mCrv.mNode,self.mNode)
...
def rig_skeleton(self):
    ...
    if self.b_lever:
        ...
        mRigNull.connectChildNode(ml_rigJoints[0],'rigLeverJoint','rigNull')
        mRigNull.connectChildNode(ml_fkJoints[0],'fkLeverJoint','rigNull')
    ...
    try:
        ...
        if mBlock.ikSetup:
            ...
            if mPivotHelper:
                ...
                raise ValueError,"Don't have ik handle creation for here yet..."
...
def rig_frame(self):
    try:
        ...
        mPivotHolderHandle = ml_templateHandles[-1]
        if mPivotHolderHandle.getMessage('pivotHelper'):
            log.info("|{0}| >> Pivot setup initial".format(_str_func))
...
def rig_cleanUp(self):
    ...
    ml_targetDynParents = [self.mConstrainNull]#[self.md_dynTargetsParent['driverPoint']]#[self.mDeformNull]
    
    if not self.mConstrainNull.hasAttr('cgmAlias'):
        self.mConstrainNull.addAttr('cgmAlias','{0}_base'.format(self.d_module['partName']))
