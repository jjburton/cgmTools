        ...
        for i,mObj in enumerate(ml_handles):
            ...
            if _mLoft:
                if loftHandles:
                    if loftHandleMode == 'local':
                        mObj.translate = dat_loft[i]['trans']
                        mObj.rotate = dat_loft[i]['rot']                        
                    else:
                        mObj.p_position = dat_loft[i]['pos']
                        mObj.p_orient = dat_loft[i]['orient']                    
                        
                    #mObj.p_position = dat_loft[i]['pos']
                    #mObj.scale = dat_loft[i]['scale']                
            ...
            if _mSubShapers:
                ...
                if loftHandles:
                    ...
                    #Relative set
                    relativePointDat_getFromObjs([mObj.mNode for mObj in _mSubShapers],ml_handles[i].mNode,ml_handles[i+1].mNode, dat_subRel[i] ) 
...
def dat_get(mBlock=None):
    ...
    for i,mObj in enumerate(ml_handles):
        ...
        #Get our loft curve and process if we have it
        mLoftCurve = mObj.getMessageAsMeta('loftCurve')
        
        if mLoftCurve:
            _d['loft'].append(get(mLoftCurve))
            _d['loftShapes'].append(shapes_get(mLoftCurve))
        else:
        ...
        if ml_subShapers:
            ...
            for ii,mObj in enumerate(ml_subShapers):
                l_subShapes.append( shapes_get(mObj))
            
            _d['sub'].append( relativePointDat_getFromObjs([mObj.mNode for mObj in ml_subShapers],
