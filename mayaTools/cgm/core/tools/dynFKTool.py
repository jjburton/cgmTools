...
def buildColumn_main(self,parent, asScroll = False):
    ...
    self.itemList = cgmUI.cgmScrollList(_inside, numberOfRows = 8, height=100)
    self.itemList(edit=True, allowMultiSelection=False)

    cgmUI.add_Button(_inside,'Add Selected',
                     cgmGEN.Callback(uiFunc_add_selected,self),
                     "Load first selected object.")

    cgmUI.add_LineSubBreak()

    cgmUI.add_Button(_inside,'Make Dynamic Chain',
                     cgmGEN.Callback(uiFunc_make_dynamic_chain,self),
                     "Load first selected object.")

    cgmUI.add_LineSubBreak()

    self.optionsFrame = mUI.MelFrameLayout(_inside, label="Options", collapsable=True, collapse=True)
