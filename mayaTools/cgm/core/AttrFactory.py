...
from cgm.core.ObjectFactory import *
...
class AttrFactory():
    ...
    def __init__(self,objName,attrName,attrType = False,value = None,enum = False,initialValue = None,lock = None,keyable = None, hidden = None, *a, **kw):
        ...
        *a, **kw
        ...
        if mc.objExists('%s.%s'%(self.obj.mNode,attrName)):
            ...
            if not attributes.validateAttrTypeMatch(attrType,currentType) and self.form is not False:
                if self.obj.refState:
                    return guiFactory.warning("'%s' is referenced. cannot convert '%s' to '%s'!"%(self.obj.mNode,attrName,attrType))                   
            ...
            try:
                ...
                guiFactory.warning("'%s.%s' failed to add"%(self.obj.mNode,attrName))
        ...
        #guiFactory.report("'%s.%s' >> '%s' >> is '%s'"%(self.obj.mNode,self.attr,self.value,self.form))
    ...
    def doConvert(self,attrType):
        ...
        self.updateData()
        if self.obj.refState:
            return guiFactory.warning("'%s' is referenced. cannot convert '%s' to '%s'!"%(self.obj.mNode,self.attr,attrType))                           
        ...
        guiFactory.warning("'%s.%s' converted to '%s'"%(self.obj.mNode,self.attr,attrType))
            
        """except:
            guiFactory.warning("'%s.%s' failed to convert"%(self.obj.mNode,self.attr))"""
    ...
    def set(self,value,*a, **kw):
        ...
        *a, **kw
        ...
        try:
            ...
            guiFactory.warning("'%s.%s' failed to set '%s'"%(self.obj.mNode,self.attr,value))
    ...
    def get(self,*a, **kw):
        ...
        *a, **kw
        ...
        try:
            ...
            #guiFactory.report("'%s.%s' >> '%s'"%(self.obj.mNode,self.attr,self.value))
            return self.value
        except:
            guiFactory.warning("'%s.%s' failed to get"%(self.obj.mNode,self.attr))
    ...
    def getMessage(self,*a, **kw):
        ...
        *a, **kw
        ...
        try:
            ...
            guiFactory.report("'%s.%s' >Message> '%s'"%(self.obj.mNode,self.attr,self.value))
            return self.value
    ...
    def setEnum(self,enumCommand):
        ...
        try:
            if self.form == 'enum':
                ...
                guiFactory.report("'%s.%s' has been updated!"%(self.obj.mNode,self.attr))
                
            else:
                guiFactory.warning("'%s.%s' is not an enum. Invalid call"%(self.obj.mNode,self.attr))
            
        except:
            guiFactory.warning("'%s.%s' failed to change..."%(self.obj.mNode,self.attr))
    ...
    def doStore(self,infoToStore,convertIfNecessary = True):
        ...
        try:
            ...
            guiFactory.warning("'%s.%s' failed to store '%s'"%(self.obj.mNode,self.attr,infoToStore))
    ...
    def doDelete(self):
        ...
        try:
            ...
            guiFactory.warning("'%s.%s' deleted"%(self.obj.mNode,self.attr))
            self.value = None
            return self.value
        
        except:
            guiFactory.warning("'%s.%s' failed to delete"%(self.obj.mNode,self.attr))  
    #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # Set Options
    #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>                       
            ...
            if value is not None:
                if self.children:
                    ...
                    try:
                        ...
                        guiFactory.warning("'%s.%s' failed to set a default value"%(self.obj.mNode,self.attr))       
        ...
        if self.numeric and not self.children: 
            if value is False:
                try:
                    ...
                    self.maxValue = value
                    guiFactory.warning("'%s.%s' had it's max value cleared"%(self.obj.mNode,self.attr))                     
                except:
                    guiFactory.warning("'%s.%s' failed to clear a max value"%(self.obj.mNode,self.attr))  
                ...
                try:
                    ...
                    guiFactory.warning("'%s.%s' failed to set a max value"%(self.obj.mNode,self.attr))
        ...
        if self.numeric and not self.children: 
            if value is False:
                try:
                    ...
                    self.minValue = value
                    guiFactory.warning("'%s.%s' had it's min value cleared"%(self.obj.mNode,self.attr))                     
                except:
                    guiFactory.warning("'%s.%s' failed to clear a min value"%(self.obj.mNode,self.attr))
        ...
        if self.numeric and not self.children: 
            if value is False:
                try:
                    ...
                    guiFactory.warning("'%s.%s' had it's soft max value cleared"%(self.obj.mNode,self.attr))                     
                except:
                    guiFactory.warning("'%s.%s' failed to clear a soft max value"%(self.obj.mNode,self.attr))  
                ...
                try:
                    ...
                    guiFactory.warning("'%s.%s' failed to set a soft max value"%(self.obj.mNode,self.attr))
            ...
            if self.children:
                ...
                for c in self.children:
                    ...
                    if not cInstance.locked:
                        ...
                        guiFactory.report("'%s.%s' locked!"%(cInstance.obj.mNode,cInstance.attr))
                        cInstance.locked = True
                self.updateData()  
                
            elif not self.locked:
                mc.setAttr((self.obj.mNode+'.'+self.attr),e=True,lock = True) 
                guiFactory.report("'%s.%s' locked!"%(self.obj.mNode,self.attr))
                self.locked = True
            ...
            if self.children:
                ...
                for c in self.children:
                    ...
                    if cInstance.locked:
                        ...
                        guiFactory.report("'%s.%s' unlocked!"%(cInstance.obj.mNode,cInstance.attr))
                        cInstance.locked = False
                self.updateData()  
                
            elif self.locked:
                mc.setAttr((self.obj.mNode+'.'+self.attr),e=True,lock = False)           
                guiFactory.report("'%s.%s' unlocked!"%(self.obj.mNode,self.attr))
                self.locked = False
            ...
            if self.children:
                ...
                for c in self.children:
                    ...
                    if not cInstance.hidden:
                        ...
                        guiFactory.report("'%s.%s' hidden!"%(cInstance.obj.mNode,cInstance.attr))
                ...
                guiFactory.report("'%s.%s' hidden!"%(self.obj.mNode,self.attr))
                self.hidden = True
            ...
            if self.children:
                ...
                for c in self.children:
                    ...
                    if cInstance.hidden:
                        ...
                        guiFactory.report("'%s.%s' unhidden!"%(cInstance.obj.mNode,cInstance.attr))
                        cInstance.hidden = False
                
            elif self.hidden:
                mc.setAttr((self.obj.mNode+'.'+self.attr),e=True,channelBox = True)           
                guiFactory.report("'%s.%s' unhidden!"%(self.obj.mNode,self.attr))
                self.hidden = False
            ...
            if self.children:
                ...
                for c in self.children:
                    ...
                    if not cInstance.keyable:
                        ...
                        guiFactory.report("'%s.%s' keyable!"%(cInstance.obj.mNode,cInstance.attr))
                ...
                guiFactory.report("'%s.%s' keyable!"%(self.obj.mNode,self.attr))
                self.keyable = True
                self.hidden = False
            ...
            if self.children:
                ...
                for c in self.children:
                    ...
                    if cInstance.keyable:
                        ...
                        guiFactory.report("'%s.%s' unkeyable!"%(cInstance.obj.mNode,cInstance.attr))
                ...
                guiFactory.report("'%s.%s' unkeyable!"%(self.obj.mNode,self.attr))
                self.keyable = False
        ...
        if arg:
            try:
                if arg != self.nameAlias:
                    ...
                    guiFactory.report("'%s.%s' already has that alias!"%(self.obj.mNode,self.attr,arg))
                    
            except:
                guiFactory.warning("'%s.%s' failed to set alias of '%s'!"%(self.obj.mNode,self.attr,arg))
    ...
    def doNiceName(self,arg):
        ...
        if arg:
            try:
                ...
                guiFactory.warning("'%s.%s' failed to set nice name of '%s'!"%(self.obj.mNode,self.attr,arg))
        ...
        if arg:
            try:
                if arg != self.nameLong:
                    ...
                    guiFactory.report("'%s.%s' already has that nice name!"%(self.obj.mNode,self.attr,arg))
                    
            except:
                guiFactory.warning("'%s.%s' failed to rename name of '%s'!"%(self.obj.mNode,self.attr,arg))
