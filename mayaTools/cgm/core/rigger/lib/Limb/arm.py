...
def build_rigSkeleton(goInstance = None):
	...
	def build_chains(self):#================================================================================
	    ...
	    except Exception,error: raise StandardError,"Rig chain | %s"%error
	    
	    try:#Blend Chain 
		self.ml_blendJoints = self._go.build_handleChain('blend','blendJoints')	   
		
	    except Exception,error: raise StandardError,"Blend chain | %s"%error
	    ...
	    except Exception,error: raise StandardError,"Ik chain | %s"%error
	...
	def build_influenceChains(self):#=======================================================================
		...
		raise StandardError,"Don't have 3 influence joints: '%s'"%(self.ml_segmentHandleJoints)
...
def build_rigSkeleton2(self): 
	...
	for i_obj in ml_fkJoints:
		...
		i_obj.rotateOrder = self._jointOrientation
...
def build_shapes(goInstance = None):
	...
	def build_shapes(self):
	    ...
	    except Exception,error: raise StandardError,"Failed to get influence chains | %s"%error
	    if not ml_influenceChains:raise StandardError,"Found no influence chains"
	    try:
		ml_segmentIKShapes = []
	    ...
	    except Exception,error: raise StandardError,"Failed to generate initial shapes | %s"%error
...
def build_controls(goInstance = None):
	...
	def verify(self):  
		...
		raise StandardError, "Must be shaped"	    
		...
		for i in range(50):
			...
			break  
	    except Exception,error: raise StandardError,"Get segment shapes | %s"%error
	    if not self.ml_segmentIKChains:raise StandardError,"No segment chains found"
	...
	def build_fk(self):
		...
		raise StandardError,"Must have at least three fk controls"	    
...
def build_FKIK(goInstance = None):
	...
	def build_connections(self):
	    ...
	    self.mPlug_FKon = cgmMeta.cgmAttr(self.mi_settings,'result_FKon',attrType='float',defaultValue = 0,keyable = False,lock=True,hidden=False)	
	    self.mPlug_IKon = cgmMeta.cgmAttr(self.mi_settings,'result_IKon',attrType='float',defaultValue = 0,keyable = False,lock=True,hidden=False)	
...
def build_FKIK2(self):
	...
	log.error("arm.build_FKIK>>bad self!")
	raise StandardError,error
...
def build_deformation(goInstance = None):
	...
	def verify(self):      
		...
		raise StandardError, "Too many segment chains, not a regular leg."
	    
	    #>>>Influence Joints
	    self.ml_influenceChains = self._go._get_influenceChains()
	    if len(self.ml_influenceChains)!=len(self.ml_segmentChains):
		raise StandardError, "Segment chains don't equal segment influence chains"
	...
	def build_segments(self):
		...
		try:#We're gonna attach to the blend chain
			...
			mc.pointConstraint(self._go._i_rigNull.mainSegmentHandle.mNode,mi_segmentAnchorEnd.mNode)
		    else:
		    ...
		    log.error("Failed to connect anchor: %s"%(mi_segmentAnchorStart.getShortName()))
		    raise StandardError,error
...
def build_rig(goInstance = None):
	...
	def build_dynParentGroups(self):
		...
		raise StandardError,"hand ik dynamic parent setup fail | %s "%error
		...
		raise StandardError,"elbow ik dynamic parent setup fail | %s "%error
		...
		raise StandardError," fk shoulder | %s "%error
	...
	def build_rigStructure(self):
	    ...
	    #Ik Scale Object
	    for obj in ml_ikJoints[2:]:
		cgmMeta.cgmAttr(self.mi_controlIK,'scale').doConnectOut("%s.scale"%obj.mNode)
	    for attr in ['x','z']:
		cgmMeta.cgmAttr(self.mi_controlIK,'s%s'%self.orientation[1]).doConnectOut("%s.s%s"%(self.mi_controlIK.mNode,attr))
	    
	    attributes.doSetLockHideKeyableAttr(self.mi_controlIK.mNode,lock=True,visible=False,keyable=False,channels=['sz','sx'])    
	    ...
	    #FK Scale
	    attributes.doSetLockHideKeyableAttr(self.ml_controlsFK[-1].mNode,lock=False,visible=True,keyable=True,channels=['s%s'%self.orientation[0]])
	    for attr in self.orientation[1:]:
		cgmMeta.cgmAttr(self.ml_controlsFK[-1],'s%s'%self.orientation[0]).doConnectOut("%s.s%s"%(self.ml_controlsFK[-1].mNode,attr))
			    
	    mPlug_fkHandScale = cgmMeta.cgmAttr(self.ml_controlsFK[-1],'s%s'%self.orientation[0])
...
def build_rig2(self):
	...
	log.error("arm.build_rig>> Gather data fail!")
	raise StandardError,error
	...
	if mi_controlIK.getMessage('spacePivots'):
	    ml_handDynParents.extend(mi_controlIK.msgList_get('spacePivots',asMeta = True))	
	...
	if mi_controlIK.getMessage('spacePivots'):
	    ml_elbowDynParents.extend(mi_controlIK.msgList_get('spacePivots',asMeta = True))
    ...
    if ml_rigHandleJoints[-1].getMessage('scaleJoint'):#If our wrist has a scale joint, we need to connect it to our last joint of our last segment
	attributes.doConnectAttr((ml_segmentChains[-1][-1].mNode + '.s'),(ml_rigHandleJoints[-1].scaleJoint.mNode+'.s'))
    ...
    for attr in ['x','z']:
	cgmMeta.cgmAttr(mi_controlIK,'s%s'%orientation[1]).doConnectOut("%s.s%s"%(mi_controlIK.mNode,attr))
    
    attributes.doSetLockHideKeyableAttr(mi_controlIK.mNode,lock=True,visible=False,keyable=False,channels=['sz','sx'])    
    mPlug_ikHandScale = cgmMeta.cgmAttr(mi_controlIK,'s%s'%orientation[1])
    mPlug_ikHandScale.p_nameAlias = 'ikScale'
    mPlug_ikHandScale.p_keyable = True

    #FK Scale
    attributes.doSetLockHideKeyableAttr(ml_controlsFK[-1].mNode,lock=False,visible=True,keyable=True,channels=['s%s'%orientation[0]])
    for attr in orientation[1:]:
	cgmMeta.cgmAttr(ml_controlsFK[-1],'s%s'%orientation[0]).doConnectOut("%s.s%s"%(ml_controlsFK[-1].mNode,attr))
	
    #cgmMeta.cgmAttr(ml_controlsFK[-1],'scale').doConnectOut("%s.scale"%ml_controlsFK[-1].mNode)
    #cgmMeta.cgmAttr(ml_controlsFK[-1],'scale').doConnectOut("%s.inverseScale"%ml_controlsFK[-1].mNode)
    
    mPlug_fkHandScale = cgmMeta.cgmAttr(ml_controlsFK[-1],'s%s'%orientation[0])
    mPlug_fkHandScale.p_nameAlias = 'fkScale'
    mPlug_fkHandScale.p_keyable = True
    
    #Blend the two
    mPlug_FKIK = cgmMeta.cgmAttr(mi_settings.mNode,'blend_FKIK')
    rUtils.connectBlendJointChain(ml_fkJoints[-1],ml_ikJoints[2:],ml_blendJoints[2:],
                                  driver = mPlug_FKIK.p_combinedName,channels=['scale'])    
...
def build_twistDriver_shoulder(goInstance = None):
	...
	def verify(self):      
		...
		raise StandardError,"failed to create stable rotate group: %s"%(error)
		...
		raise StandardError,"failed to create ik handle: %s"%(error)
...
def build_twistDriver_shoulder2(self):
    log.info(">>> %s.build_twistDriver_shoulder >> "%self._strShortName + "="*75)
    try:
	if not self._cgmClass == 'RigFactory.go':
	    log.error("Not a RigFactory.go instance: '%s'"%self)
	    raise StandardError
    except StandardError,error:
	log.error("build_twistDriver_shoulder>>bad self!")
	raise StandardError,error
	...
	raise StandardError,"%s.build_twistDriver_shoulder >> Failed joint creation,positioning | %s"%(self._strShortName,error)	    
	...
	raise StandardError,"%s.build_twistDriver_shoulder >> failed to create ik handle: %s"%(self._strShortName,error)
...
def build_twistDriver_wrist(goInstance = None):
	...
	def verify(self):      
		...
		if i_targetJoint.cgmName != 'wrist':
		    raise StandardError,"target not wrist? | %s"%(i_targetJoint.p_nameShort)	
		if i_blendWrist.cgmName != 'wrist':
		    raise StandardError,"blend target not wrist? | %s"%(i_blendWrist.p_nameShort)	
...
def build_twistDriver_wrist2(self):
    log.info(">>> %s.build_twistDriver_wrist >> "%self._strShortName + "="*75)
    try:
	if not self._cgmClass == 'RigFactory.go':
	    log.error("Not a RigFactory.go instance: '%s'"%self)
	    raise StandardError
    except StandardError,error:
	log.error("build_twistDriver_wrist>>bad self!")
	raise StandardError,error
	...
	raise StandardError,"%s.build_twistDriver_wrist >> failed to setup start attr | %s"%(self._strShortName,error)	
    """
    try:
	mi_parentRigNull = self._i_module.moduleParent.rigNull
	...
	i_blendWrist = ml_blendJoints[2]
	if i_targetJoint.cgmName != 'wrist':
	    raise StandardError,"%s.build_twistDriver_wrist >> target not wrist? | %s"%(self._strShortName,i_targetJoint.p_nameShort)	
	if i_blendWrist.cgmName != 'wrist':
	...
	raise StandardError,"%s.build_twistDriver_wrist >> Failed joint creation,positioning | %s"%(self._strShortName,error)	    
	...
	raise StandardError,"%s.build_twistDriver_wrist >> failed to create ik handle: %s"%(self._strShortName,error)
       
    #>>> Control	
    try:
	#>>> Connect in
	cgmMeta.cgmAttr(self._i_module.rigNull.settings,'in_worldIKEnd').doConnectIn("%s.r%s"%(i_startRoot.mNode,self._jointOrientation[0]))
	self.connect_toRigGutsVis(ml_twistObjects)#connect to guts vis switches
    except StandardError,error:
...
def __build__(self, buildTo='',*args,**kws): 
    """
    For the arm, build order is skeleton first as we need our mid segment handle joints created to cast from
    """
    try:
	if not self._cgmClass == 'RigFactory.go':
	    log.error("Not a RigFactory.go instance: '%s'"%self)
