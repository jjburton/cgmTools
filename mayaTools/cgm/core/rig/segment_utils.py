...
def create_curveSetup(jointList = None,
    ...
    #>> Orient ========================================================================================
    if not mi_module:#if it is, we can assume it's right
    ...
    if b_addMidTwist:#We need to get our factors
        ...
        #log_info("beforeJoints: %s"%[i_jnt.getShortName() for i_jnt in ml_beforeJoints])
        #log_info("afterJoints: %s"%[i_jnt.getShortName() for i_jnt in ml_afterJoints])           
...
def create_segment_curveBAK(*args,**kws):
    class fncWrap(cgmGEN.cgmFuncCls):
        ...
        def _verify_(self):
            try:#Query ===========================================================================================
                ...
                self.mi_mayaOrientation = VALID.simpleOrientation(self.d_kws['orientation'])
                self.str_orientation = self.mi_mayaOrientation.p_string
                self.str_secondaryAxis = VALID.stringArg(self.d_kws['secondaryAxis'],noneValid=True)
                self.str_baseName = VALID.stringArg(self.d_kws['baseName'],noneValid=True)
                self.str_connectBy = VALID.stringArg(self.d_kws['connectBy'],noneValid=True)		
                self.b_addMidTwist = VALID.boolArg(self.d_kws['addMidTwist'])
                self.b_advancedTwistSetup = VALID.boolArg(self.d_kws['advancedTwistSetup'])
                self.b_extendTwistToEnd= VALID.boolArg(self.d_kws['extendTwistToEnd'])
            ...
            try:#Validate =====================================================================================
                ...
                #Good way to verify an instance list? #validate orientation             
                #> axis -------------------------------------------------------------
                self.axis_aim = VALID.simpleAxis("%s+"%self.str_orientation [0])
                self.axis_aimNeg = VALID.simpleAxis("%s-"%self.str_orientation [0])
                self.axis_up = VALID.simpleAxis("%s+"%self.str_orientation [1])
        ...
        def _curveCheck_(self):
            try:#Query ===========================================================================================
                if self.mi_useCurve:
                    #must get a offset u position
                    self.f_MatchPosOffset = CURVES.getUParamOnCurve(self.ml_joints[0].mNode, self.mi_useCurve.mNode)
            except Exception,error:raise Exception,"[Query | error: {0}".format(error)  
        ...
        def _func_verify_(self):
            try:#Query ===========================================================================================
                ...
                if _controlOrientation is None:
                    ...
                    self.mi_mayaControlOrientation = VALID.simpleOrientation(self.d_kws['controlOrientation'])
                ...
                self.str_baseName = VALID.stringArg(self.d_kws['baseName'],noneValid=True)
                self.str_connectBy = VALID.stringArg(self.d_kws['connectBy'],noneValid=True)	
                self.b_addTwist = VALID.boolArg(self.d_kws['addTwist'])                
                self.b_addMidTwist = VALID.boolArg(self.d_kws['addMidTwist'])
                self.b_advancedTwistSetup = VALID.boolArg(self.d_kws['advancedTwistSetup'])
                self.b_addSquashStretch = VALID.boolArg(self.d_kws['addSquashStretch'])
                self.b_additiveScaleSetup = VALID.boolArg(self.d_kws['additiveScaleSetup'])
                self.b_additiveScaleConnect = VALID.boolArg(self.d_kws['additiveScaleConnect'])
...
def createCGMSegmentOLD(jointList, influenceJoints = None, addSquashStretch = True, addTwist = True,
    ...
    try:#Gather data =====================================================================================
        ...
        axis_up = VALID.simpleAxis("%s+"%orientation[0])
