...
AcgmRVS_TO_EXPAND = {'max': 'maximum',
				  'min': 'minimum'}

def camelCaseToNice( theString, acgmreviationsToExpand=None, niceParityNames=True ):
	...
	if niceParityNames:
		...
		if parity is not Parity.NONE:
			...
			if acgmreviationsToExpand is None: acgmreviationsToExpand = {}
			acgmreviationsToExpand[ asName._parityStr ] = niceParityStr

	if acgmreviationsToExpand is None:
		words = [ w.capitalize() for w in words ]
	else:
		words = [ acgmreviationsToExpand.get(w.lower(), w).capitalize() for w in words ]
