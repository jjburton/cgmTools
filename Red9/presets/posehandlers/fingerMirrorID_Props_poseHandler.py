...
incprops = True  # do we include the Props in the finger data?

def getAdditionalCtrls(fingersystem):
	'''
	new overload for the finger Pose systems so they can include the Prop Ctrls in the dataset
	'''
	armSystem = fingersystem.getParentMetaNode()
	if armSystem:
		return armSystem.getChildren(cAttrs=['CTRL_Prop','CTRL_*HandIdleController'])
	return []
    ...
    if not issubclass(type(metaNode), r9Meta.MetaHIKControlSetNode):
        ...
        if not currentSelection:
            result = cmds.confirmDialog(
                                ...
                                button=['L_Fingers', 'R_Fingers', 'L_Toes', 'R_Toes'],
...
#             filteredNodes = metaNode.R_ArmSystem.R_FingerSystem.getChildren()
    ...
    if poseObj.useFilter:
		if incprops:
			# now include props
			ctrls = fingersystem.getChildren()
			props = getAdditionalCtrls(fingersystem)
			if props:
				ctrls.extend(props)
				print(('Finger : PoseOverload Handler : >> Props Being included: %s' % [r9Core.nodeNameStrip(prop) for prop in props]))
			return ctrls
		else:		
